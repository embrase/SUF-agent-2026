...
export function handleListAgents(db: Firestore) {
  return async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      console.error('[admin/agents] Failed to list agents:', error);
      sendError(res, 500, 'internal_error', error.message || 'Failed to list agents');
    }
  };
}
...
export function handleGetAgent(db: Firestore) {
  return async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      console.error('[admin/agents] Failed to get agent:', error);
      sendError(res, 500, 'internal_error', error.message || 'Failed to get agent');
    }
  };
}
...
export function handleSuspendAgent(db: Firestore) {
  return async (req: AdminAuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      console.error('[admin/agents] Failed to suspend agent:', error);
      sendError(res, 500, 'internal_error', error.message || 'Failed to suspend agent');
    }
  };
}
...
export function handleResetAgentKey(db: Firestore) {
  return async (req: AdminAuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      console.error('[admin/agents] Failed to reset agent key:', error);
      sendError(res, 500, 'internal_error', error.message || 'Failed to reset agent key');
    }
  };
}
